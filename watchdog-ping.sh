...
IP=$(echo -e "$PING" | grep PING | awk '{ print $3 }' | tr -d '(' | tr -d ')')
...
if [ "$PERCENT_LOST" == "100" ]
then
mysql -u $USER -p$PASSW -D $DATABASE -e "insert INTO ping(ip,status,tx,rx,percent_lost,min,avg,max,mdev) values(\"$IP\",\"FAIL\",\"$TX\",\"$RX\",\"$PERCENT_LOST\",\"$MIN\",\"$AVG\",\"$MAX\",\"$MDEV\");"
##### Mail notify ################### 
#<In my script i use mailgun to send me an email, Replace this line with your method to send email>
#####################################
else
mysql -u $USER -p$PASSW -D $DATABASE -e "insert INTO ping(ip,status,tx,rx,percent_lost,min,avg,max,mdev) values(\"$IP\",\"OK\",\"$TX\",\"$RX\",\"$PERCENT_LOST\",\"$MIN\",\"$AVG\",\"$MAX\",\"$MDEV\");"
fi
